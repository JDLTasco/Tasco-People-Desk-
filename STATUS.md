...
- Current stage: 7 — Infrastructure, essentially complete. **Azure infra deployed and live** in `rg-tasco-people-desk` (Australia East): App Service, PostgreSQL Flexible Server, Blob Storage (Defender enabled), Key Vault, Application Insights, Event Grid system topic + scan-results event subscription, 7 Logic App job timers. **App code deployed and live** at `hr.tascopetroleum.com.au` (real custom domain, SSL bound, DNS resolving) — `/api/health` returns `{"status":"ok"}`. RBAC role assignments, DB migration/seed/app_role-password, and the Event Grid scan-results subscription are all confirmed done. **§14 Track A (DNS/cert) and Track B (Entra app registration + 3 security groups) done and wired live 2026-09-19**: `AZURE_AD_CLIENT_ID`/`_TENANT_ID`/the 3 `AZURE_AD_GROUP_*_ID` settings and the `AZURE-AD-CLIENT-SECRET` Key Vault secret are all set; the Entra app's redirect URI matches (`az ad app show`); `groupMembershipClaims` (which was never configured — a real Entra gap, not a code issue) is now set to `SecurityGroup` by John once he got Entra owner rights. **A real, previously-undetected code bug was found and fixed this session, not just config**: `lib/auth.ts`'s `jwt` callback read `account.profile.groups`, but NextAuth passes `profile` as its own separate callback parameter — `account.profile` was always `undefined`, so `deriveRole()` always returned `null` and the callback silently never set `token.role`/`userId`/`entraObjectId` for any real Azure AD sign-in. Invisible until today because every prior test used the dev-mock provider (a different code path); found when John signed in successfully but showed no role/no admin access. **Fixed in `lib/auth.ts` (destructure `profile` directly), deployed and live** — awaiting John's re-test to confirm role now resolves correctly. **Azure account RBAC (Key Vault, Storage) for John's own account is also fully resolved** (Michael granted `Key Vault Administrator`+`Contributor` and `Storage Blob Data Contributor` at subscription scope, re-verified live). **Deploy reliability finding, 2026-09-19**: this app's remote Oryx build (`npm install` + `next build`, TypeScript+webpack+Prisma) is genuinely memory-hungry and shares RAM with the live running app — B1 (1.75GB) and even B2 (3.5GB) both failed repeatedly mid-build with no error, just the container going silent and getting killed (classic OOM signature, confirmed by pulling the real deployment log via `az webapp log download`, not just guessed). **B3 (7GB) succeeded reliably.** Working pattern for now: scale to B3 immediately before a code deploy, deploy, scale back to B1 once confirmed live via `/api/health`. **Worth a permanent decision, not decided unilaterally**: repeating this dance is real toil — consider staying on a higher tier (B2/B3/S1) permanently instead, especially since B2 turned out to be unreliable too (worked once, failed three times).
...
- Blockers / required operator actions: **(updated 2026-09-19, later same day)** (1) **RBAC role assignments: DONE.** (2) **DB migration/seed/app_role password: DONE.** (3) **Event Grid scan-results subscription: DONE.** (4) **§14 Track A (DNS/cert) and Track B (Entra app reg + groups): DONE, wired live this session** — see header. (5) **`main.bicep` has no saved parameters file** — unchanged risk, see prior note: do not run a full `az deployment group create` against this template without addressing this first; prefer targeted CLI calls (as used again this session for the Azure AD app settings). (6) **John's own Azure account data-plane RBAC gap: DONE, resolved same session.** Michael granted `Key Vault Administrator` + `Contributor` and `Storage Blob Data Contributor` at subscription scope; re-verified live (secret set/delete round-trip, container list via `--auth-mode login`). No further RBAC action needed anywhere in this deployment. (7) **DONE: `groupMembershipClaims` set, John's account confirmed in `HR-Ticketing-Admins`.** (8) **DONE: the real jwt-callback bug (see header) found and fixed** — deployed live, awaiting John's re-test. (9) **DONE: MANUAL ENTRY label gap fixed** (§15 acceptance test, was failing) — deployed live. (10) **DONE: deploy reliability root-caused** (OOM on B1/B2, B3 works — see header); no code issue. (11) Still unconfirmed: whether §14's underlying mailbox migration (`HR_MAILBOX_ID`, Graph ingestion) is actually done — separate from the app registration/groups/DNS work confirmed done this session. (12) Still deferred by John's own choice: the legal-hold/retention-purge §15 tests remain unverified against real matching data — wait for Stage 9 rather than planting fixture data in production now.
- Recommended next command or task: **John to retry sign-in at `https://hr.tascopetroleum.com.au/sign-in` and confirm the nav bar now shows his role (ADMIN) and the Admin/Categories/etc. links appear** — this is the one thing not yet confirmed after today's jwt-callback fix. Once confirmed: confirm mailbox/Graph ingestion readiness (Blockers item 11) and set `HR_MAILBOX_ID`/`GRAPH_WEBHOOK_CLIENT_STATE` if ready; re-verify Stage 7's Durability acceptance tests (restore test, blob soft-delete recovery) against real data; exercise the attachment-scanning pipeline end-to-end for the first time; when Stage 9 is nominated, plan the legal-hold/retention-purge fixture test; decide whether to permanently bump the App Service Plan tier given the deploy-reliability finding.
