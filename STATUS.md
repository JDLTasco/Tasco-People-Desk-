...
## Stage 8 (CI/CD part only) nominated: GitHub Actions auto-deploy with OIDC (2026-09-24)

**John nominated the CI/CD half of Stage 8 only** -- CSP headers, webhook rate-limit review, App Insights correlation propagation, npm audit remediation and the `xmllint` XSD check are **not** part of this and remain open. Motivation: until now a `git push` shipped nothing; every deploy was a manual Cloud Shell `az webapp deploy` plus a temporary B3 scale-up, because Oryx's remote build OOMs on B1/B2.

**Built**: `.github/workflows/deploy.yml` -- on every push to `main` (or manual run): `npm ci` -> `prisma generate` -> unit tests -> `next build` (also type-checks + lints) -> `npm prune --omit=dev` -> zip -> `azure/login@v2` (OIDC, no stored password/publish profile) -> make sure the App Service config matches (see below) -> `azure/webapps-deploy@v3` -> poll `https://hr.tascopetroleum.com.au/api/health` for a 200 (up to 10 min). Building on GitHub's runner removes the need for the B3 scale-up entirely, and the runner is outside Tasco's network, so no Sophos/gateway workaround applies. The runner (Ubuntu, OpenSSL 3) matches the App Service's Debian bookworm image (confirmed from a past Oryx log's `OS Type: bookworm`), so Prisma's native engines built there run on the App Service unchanged.

**Migrations now run automatically on app startup** (John's explicit choice via AskUserQuestion, over keeping them manual): startup command is now `node node_modules/prisma/build/index.js migrate deploy && node node_modules/next/dist/bin/next start -p 8080`, using the `DATABASE_URL` the App Service already resolves from Key Vault -- the migration-role secret never goes to GitHub. `migrate deploy` is forward-only (§0.1.7) and a no-op when nothing is pending. `prisma` moved from devDependencies to dependencies for this (lockfile change was just removing 35 `devOptional` flags, nothing re-resolved). Direct `node node_modules/...` paths rather than `.bin` shims, since the package is now zipped by us, not installed in place by Oryx.

**Config changes, in both places**: `infra/modules/appservice.bicep` now declares `SCM_DO_BUILD_DURING_DEPLOYMENT=false` and the new `appCommandLine`; the workflow's "Ensure App Service config" step applies the same two values to the live app on its first run (only writes if different, since each write restarts the app). The live app still has the old values (`true` / `next start -p 8080`) until that first run -- **a manual Cloud Shell deploy of a source-only `deploy.zip` will no longer work once the workflow has run** (nothing would build it); use the workflow's "Run workflow" button instead.

**Operator-only setup required before the first run (§0.1.5 -- tenant mutations are not Claude-performable)**: an Entra app registration for the deploy identity, a federated credential trusting `repo:JDLTasco/Tasco-People-Desk-:ref:refs/heads/main`, a `Website Contributor` role assignment scoped to the `tasco-people-desk` web app only, and three GitHub repo secrets (`AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID` -- identifiers, not keys). The workflow file itself probably has to be added through GitHub's web UI, because the local PAT lacks `workflow` scope (same as Tasco BSC). **Not yet run live -- nothing in this entry has been verified against real Azure yet.**

**Progress, same day (first three runs):** the workflow file was added via GitHub's web UI (commit `d440c6f`). Run 1 failed at `next build`'s "Collecting page data" -- `lib/prisma.ts` threw at import without `APP_DATABASE_URL` (Oryx builds had the App Service settings; CI deliberately doesn't). Run 2 failed the same way inside the unit tests (`writer.test`/`verdict.test` import it transitively; locally `.env` is auto-loaded by `@prisma/client`, which is why this never showed on this machine). Fixed properly in `dbefdd9`: the Prisma client is now created lazily behind a Proxy on first real use -- same error, still never falls back to `DATABASE_URL`. Verified with no `.env`: 190/190 tests, clean build; with `.env`: health, model delegates, batch + interactive `$transaction` all work. Also fixed in `8842bfb`: **`/api/health` had been statically prerendered at build time** (argument-less GET) -- it returned a baked-in response, never touching the DB after deploy; now `force-dynamic`. This is likely the real explanation for the mailbox-smoke-test entry's "loose end" about health never changing between deploys. Run 3 passed tests + build and stopped at `azure/login` (GitHub secrets not visible to the run -- John re-checking them).

**Current blocker (2026-09-24): the deploy identity has no role yet.** App registration `tasco-people-desk-github-deploy` (appId `755da6b5-fa97-48a2-bc8a-f31e27c91462`, SP object `44588887-2d32-45df-be75-2f0a77c6d403`) and its federated credential (`repo:JDLTasco/Tasco-People-Desk-:ref:refs/heads/main`) exist and are correct. The `Website Contributor` assignment failed: **John's `Role Based Access Control Administrator` grant carries an ABAC condition limiting him to assigning only Key Vault Secrets User / Key Vault Secrets Officer / Storage Blob Data Contributor.** John chose (over a publish-profile fallback) to ask Michael (IT) to create the assignment, scoped to the `tasco-people-desk` web app only. After that: confirm the three repo secrets, re-run the workflow, watch it apply the config and deploy.

**RESOLVED same day -- auto-deploy is LIVE, via a temporary publish-profile fallback.** Michael was away, so John chose (option offered explicitly, deviation stated) to use the same publish-profile auth Tasco BSC uses: `AZURE_WEBAPP_PUBLISH_PROFILE` repo secret, `azure/login` + the "Ensure App Service config" step removed from the workflow (John edited it via the GitHub web UI; Claude Code's safety classifier declined to write a credential-using workflow or change production App Service config itself). John applied the two App Service settings once by hand in Cloud Shell (`SCM_DO_BUILD_DURING_DEPLOYMENT=false`, the new `appCommandLine`) -- site verified healthy after that restart. **Recorded spec deviation (§2 CI/CD row, §0.1.8): a deploy credential is stored in GitHub** until IT grants `Website Contributor` to the already-created OIDC identity; then restore `azure/login` (OIDC), drop the publish-profile secret, and consider disabling SCM basic-auth publishing again.

**First real deploy (2026-09-23T23:41Z) found one more problem**: OneDeploy runs with `CleanOutputPath False` -- new files are rsync'd over old ones, nothing removed. A stale `.next/server/app/api/health.body/.meta` from 2026-09-21 survived and Next.js kept serving it (`x-nextjs-cache: HIT`) even though the new build no longer prerenders that route; also a 220MB old `node_modules.tar.zst`, `prisma-migrate.zip`, etc. Fixed with `clean: true` on `azure/webapps-deploy` (web-UI edit `e2a69e3`). **Verified after the clean deploy (2026-09-23T23:59Z)**: stale files gone (checked via the Kudu VFS API with an Entra bearer token -- works from this machine, unlike `az webapp deploy`), `/api/health` now dynamic (no `x-nextjs-cache` header) and returns `{"status":"ok"}` -- a real DB round-trip through the new lazy Prisma client as `app_role` -- and the startup `migrate deploy` step ran cleanly (site came up). Note: Oryx still packs `node_modules` into `node_modules.tar.gz` and extracts it to `/node_modules` at startup (symlinked back into wwwroot), so the startup command's `node node_modules/...` paths resolve to the freshly deployed modules.

**How deploys work now**: push to `main` -> live in ~8 minutes, no Cloud Shell, no B3 scale-up. Manual redeploy: Actions -> "Deploy to Azure App Service" -> Run workflow. **Do not** use the old Cloud Shell `az webapp deploy` of a source-only zip any more -- Oryx build is off, so nothing would build it.

**Tooling finding worth keeping:** the long-standing `(MissingSubscription)` error from `az role assignment ...` on this machine is Git Bash (MSYS) rewriting `/subscriptions/...` arguments into Windows paths -- prefix with `MSYS_NO_PATHCONV=1` (or use PowerShell) and it works.
...
**Second real run, after the auto-reply fix, failed server-side** (not just the gateway-timeout false alarm seen on deploys): `job_runs` showed `FAILED`, `"Unterminated string in JSON at position 13301651"` -- the single `$expand=attachments` list call covering a whole day's real messages produced a many-MB response that didn't reliably transfer in one shot. Fixed: `listInboxSince` now lists message IDs only, then fetches each message (with attachments) individually via the already-tested `getMessage()` -- smaller, more resilient requests. Also made the route fire-and-forget (`202` immediately, same convention as the real Graph webhook, poll `job_runs` for the outcome), since a full import can take longer than the platform's front-end gateway timeout. 180/180 tests still passing, `tsc`/lint clean. Redeployed via the same B3-scale-up-then-down Cloud Shell pattern.

**Third real run: fully successful.** `processed: 20, created: 17, threaded: 3, suppressed: 0, ignored: 0, duplicate: 0` -- confirmed via `job_runs` (`SUCCEEDED`, `items_processed: 20`, 26 seconds, no error) and the response body. Zero false auto-reply positives (was 19/19 before the fix), zero lost/errored messages. 17 new real tickets created: `260922100401, 260922100701, 260922100901, 260922102701, 260922124901, 260922132701, 260922132801, 260922133401, 260922140201, 260922162301, 260922163601, 260922164601, 260922172601, 260923074001, 260923090301, 260923093801, 260923094701`. **Confirmed `HR_MAILBOX_ID` was still blank on the live app throughout** (checked directly after this run) -- every claim/assign/outcome/close attempt on these tickets will still fail closed exactly as before; no email has gone out or can go out from this exercise.

**One loose end, not urgent**: this third run's response body had the shape of the *old* synchronous code (`created`/`threaded`/... returned directly) rather than the new `202`/`{"accepted":true}` shape, despite the deployment history API reporting the fire-and-forget deploy as `status: 4`/`complete: true` beforehand. Since the run succeeded either way, not chased further this session -- but worth remembering next time a deploy's *behavioral* effect needs confirming: `/api/health` and a `405` on the wrong HTTP method both stayed identical across every version deployed today and don't actually prove which code is live. If this matters again, check via the Kudu VFS API's file timestamps or an intentionally-observable behavior change, not just health/method-shape checks.

**Not yet done**: John to spot-check a few of the 17 created tickets in the live Pool/dashboard to confirm the data (subject, requester, attachments) looks right end to end; decide whether to keep these real tickets in the live system (they're genuine, likely-already-resolved HR matters) or clean them up; decide with John when/whether to actually cut over `HR_MAILBOX_ID` for real ingestion now that both the auto-reply bug and the large-response fragility are fixed.
