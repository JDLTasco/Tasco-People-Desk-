...
- Passing acceptance tests: the whole **Ingestion** block of §15 now passes live via the dev-only fixture endpoint (duplicate delivery, threading, invalid clientState -> 400, suppression) except the two rows that need a real mailbox (webhook against real live traffic, delta-poll safety net against a broken real webhook) and the go-live-timestamp row (no cutover date exists yet). Most of **Attachments** passes too (`.exe` blocked, `.iso` blocked, `.zip` accepted+quarantined, TYPE_MISMATCH, no server-side extraction) except the CLEAN-status row, which needs §7.3.2's now-specified but not-yet-built scan-verdict ingestion (see below).
- Failing / pending acceptance tests: **Communications**, **Legal hold**, **Audit-and-correlation**'s admin-search row, **Archive-and-retention**, **Durability** (Stages 5-7). The two live-mailbox Ingestion rows stay pending on §14 regardless of stage; the CLEAN-status Attachments row stays pending until §7.3.2's Event Grid webhook + reconciliation job are actually built (Stage 7, needs Blob Storage/Defender to exist first).
- Architecture deviations / clarifications: **§7.3.2 "Scan verdict ingestion" added to the spec itself this session** (Event Grid webhook + 15-min reconciliation poller, fail-closed) -- a genuine spec gap (Defender's scan-verdict mechanism, undefined in §7.3.1) put to John directly and now resolved; not yet built, needs Stage 7's storage infrastructure first -- see "Defender scan-verdict mechanism" below. Also a real gap found and fixed this stage (a new table Stage 1's grant migration didn't cover -- see "Stage 4 deliverables" below).
