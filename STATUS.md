...
- Blockers / required operator actions: **(updated 2026-09-19, later same day)** (1) **RBAC role assignments: DONE.** (2) **DB migration/seed/app_role password: DONE.** (3) **Event Grid scan-results subscription: DONE.** (4) **§14 Track A (DNS/cert) and Track B (Entra app reg + groups): DONE, wired live this session** — see header. (5) **`main.bicep` has no saved parameters file** — unchanged risk, see prior note: do not run a full `az deployment group create` against this template without addressing this first; prefer targeted CLI calls (as used again this session for the Azure AD app settings). (6) **John's own Azure account data-plane RBAC gap: DONE, resolved same session.** Michael granted `Key Vault Administrator` + `Contributor` and `Storage Blob Data Contributor` at subscription scope; re-verified live (secret set/delete round-trip, container list via `--auth-mode login`). No further RBAC action needed anywhere in this deployment. (7) **DONE: John's account confirmed in `HR-Ticketing-Admins`** (verified via `az ad group member list`). **Real remaining blocker is different from what it first looked like**: the app registration's `groupMembershipClaims` was never set (see header) — needs an Entra Application/Cloud Application/Global Administrator to run `az ad app update --id ea790fc8-... --set groupMembershipClaims=SecurityGroup`, or the Portal's Token configuration → Add groups claim. John has drafted the request; no ETA. (8) Also still unconfirmed: whether §14's underlying mailbox migration (`HR_MAILBOX_ID`, Graph ingestion) is actually done — separate from the app registration/groups/DNS work confirmed done this session. (9) **New, same session: fixed the MANUAL ENTRY label gap** (§15 acceptance test, was failing) — see dated entry below; deployed live. **New, deferred by John's own choice**: the legal-hold/retention-purge §15 tests remain unverified against real matching data — wait for Stage 9 rather than planting fixture data in production now.
- Recommended next command or task: **once an Entra admin sets `groupMembershipClaims=SecurityGroup` on the app registration, retry sign-in at `https://hr.tascopetroleum.com.au/sign-in`** — should work immediately, no restart needed (this is the one real remaining item blocking a working sign-in; group membership itself is already correct). After a successful sign-in: confirm mailbox/Graph ingestion readiness (see Blockers item 8) and set `HR_MAILBOX_ID`/`GRAPH_WEBHOOK_CLIENT_STATE` if ready; re-verify Stage 7's Durability acceptance tests (restore test, blob soft-delete recovery) against real data; exercise the attachment-scanning pipeline end-to-end for the first time; when Stage 9 is nominated, plan the legal-hold/retention-purge fixture test.
