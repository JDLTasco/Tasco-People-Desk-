...
interface StartActionBody {
  version: number;
}

// §4: ALLOCATED -> IN_ACTION, set manually by the assignee/HR_LEAD/ADMIN
// when work begins. The category guard: "An attempt to move ALLOCATED ->
// IN_ACTION without a category returns HTTP 400 with a message naming the
// missing field."
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  const body = (await request.json().catch(() => ({}))) as Partial<StartActionBody>;
  ...
  const check = validateTransition(ticket.status, "IN_ACTION", {
  ...
  const result = await prisma.ticket.updateMany({
    ...
    data: { status: "IN_ACTION", version: { increment: 1 } },
  });
  if (result.count === 0) {
